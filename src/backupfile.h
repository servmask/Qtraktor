...
public:
    ...
    bool extract(QDir extractTo)
    {
        ...
        while (!atEnd()) {
            ...
            if (fullPath.isEmpty()) {
                log(QString("Error: Unsafe path in archive for file '%1'. Skipping.").arg(info.fileName), true);
            ...
            if (isEncrypted && !filePassword.isEmpty()) {
                streamOk = CryptoUtils::processFileContentWithPasswordStreaming(this, info.fileSize, &out, isCompressed,
                                                                                info.fileName, filePassword,
                                                                                compressionType, &processError);
            } else {
                streamOk = CryptoUtils::processFileContentStreaming(this, info.fileSize, &out, isCompressed,
            ...
            if (isV2 && !info.crc32.isEmpty()) {
                ...
                if (!actualCrc.isEmpty() && actualCrc != info.crc32) {
                    log(QString("CRC mismatch for file '%1': expected %2, got %3")
                            .arg(info.fileName, info.crc32, actualCrc),
                        true);
                }
            }
        }
...
private:
    ...
    static QString buildOutputPath(const QDir &extractTo, const QString &fileName, const QString &filePath)
    {
        ...
        // A malicious .wpress could use "../" in filePath or fileName to
        // write files outside the extraction directory.
        QString safePath = filePath;
        QString safeName = fileName;

        // Remove any ../ or ..\ sequences and leading slashes
        safePath.remove(QRegularExpression("\\.\\./|\\.\\\\|^/|^\\\\"));
        safeName.remove(QRegularExpression("\\.\\./|\\.\\\\|^/|^\\\\"));

        QString fullPath;
        if (safePath.isEmpty() || safePath == ".") {
            fullPath = extractTo.absolutePath() + "/" + safeName;
        } else {
            const QString dirPath = extractTo.absolutePath() + "/" + safePath;
            QDir dir(dirPath);
            if (!dir.exists()) {
            ...
            fullPath = dirPath + "/" + safeName;
        }

        // Final check: resolve the canonical parent and verify it's under extractTo
        QFileInfo info(fullPath);
        QString resolvedDir = info.absolutePath();
        QString resolvedBase = extractTo.absolutePath();
